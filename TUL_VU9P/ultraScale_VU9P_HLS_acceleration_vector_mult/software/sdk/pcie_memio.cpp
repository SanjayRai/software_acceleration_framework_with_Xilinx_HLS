...
int xDMA_througput_h2c (fpga_xDMA *my_fpga_xDMA_ptr, data_t *data_buffer);
int xDMA_througput_c2h (fpga_xDMA *my_fpga_xDMA_ptr, data_t *data_buffer);
...
int main(int argc, char **argv) {
    ...
    memset (A_IN, 0x0, MAX_IN0_MEMORY_SIZE);
    xfer_byte_count_temp = xDMA_througput_h2c (my_fpga_xDMA_ptr, A_IN);
    xfer_byte_count_temp = xDMA_througput_c2h (my_fpga_xDMA_ptr, A_IN);
...
int xDMA_througput_h2c (fpga_xDMA *my_fpga_xDMA_ptr, data_t *data_buffer) {
    int xfer_count;
    int xfer_size;
    xfer_size = MAX_IN0_MEMORY_SIZE;
    auto start_t = chrono::high_resolution_clock::now();
    xfer_count = my_fpga_xDMA_ptr->fpga_xDMA_write(DDR4_BASE, (char *)data_buffer, xfer_size);
    auto stop_t = chrono::high_resolution_clock::now();
    chrono::duration<double> elapsed_hi_res = stop_t - start_t ;
    double high_res_elapsed_time = elapsed_hi_res.count();
...
int xDMA_througput_c2h (fpga_xDMA *my_fpga_xDMA_ptr, data_t *data_buffer) {
    ...
    xfer_size = MAX_IN0_MEMORY_SIZE;
    auto start_t = chrono::high_resolution_clock::now();
    xfer_count = my_fpga_xDMA_ptr->fpga_xDMA_read(DDR4_BASE, (char *)data_buffer, xfer_size);
    auto stop_t = chrono::high_resolution_clock::now();
    chrono::duration<double> elapsed_hi_res = stop_t - start_t ;
    double high_res_elapsed_time = elapsed_hi_res.count();
